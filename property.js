...
function processData() {
    ...
    if (loan_range != "") {
        dataset = d3.nest()
            .entries(dataset.filter(function(d) {
                 var x = parseInt(loan_range)
                  var x1 = loanAmtArr.indexOf(x)
                  var x0 = x1 - 1
                  return ((parseInt(d.LoanAmountRequested) > x0) && (parseInt(d.LoanAmountRequested) <= x))
